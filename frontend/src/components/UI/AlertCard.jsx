...
export default function AlertCard({ alert, onResolve = () => {}, onIgnore = () => {} }) {
  ...
  return (
    <motion.div
      ...
      className="group rounded-2xl border border-border bg-card p-6 shadow-sm transition-all duration-200 hover:bg-accent hover:shadow-md"
    >
      <div className="flex flex-col gap-4 sm:flex-row sm:items-start">
        <div className={`${palette.bg} rounded-xl p-2 flex-shrink-0`}>
          <Icon className="h-5 w-5" />
        </div>
        <div className="flex-1 min-w-0">
          <div className="mb-2 flex items-start gap-2">
            <h3 className="text-lg font-semibold leading-tight">{alert.title}</h3>
            <Badge className={palette.bg}>{alert.severity?.toUpperCase() || 'LOW'}</Badge>
          </div>
